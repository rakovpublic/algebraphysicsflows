...
Concrete exact algebras are available through `algebra.concrete.ConcreteMathematics`, which implements the existing `IMathToolInitializer`. It registers N, Z, Q, Q(i), Boolean, prime fields, rational vectors, matrices, polynomials, rational functions, finite sets, finite relations, samples, probability measures and simplicial complexes directly in `MathTool` using implementations of the interfaces in `operations`.
