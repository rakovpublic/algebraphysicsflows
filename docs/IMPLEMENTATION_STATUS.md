...
ConcreteMathematics installs **582 operations from 34 algebra builders** in the original MathTool. Builders are in algebra/concrete; operation implementations are in operations/simple and operations/flat. Unit, rational integration bounds, polynomial iteration inputs, integer relation/category label pairs, tensor axis pairs and finite-function boundary pairs are additional supporting carriers.

Delivered families: Boolean, N, Z, Q, Q(i), rational Hamilton quaternions, prime fields, rational vectors/square matrices, variable-dimension vector and rectangular matrix families, affine solution sets, Q[x], Q(x), multivariate Poly(Q), PolynomialMap(Q), polynomial differential forms, S3, finite integer sets/relations/functions, rational samples and finite exact probability measures on integers. Operations include exact arithmetic, embeddings, scalar actions in both orders, dot products, determinant/rank/inverse, RREF and bases for nullspaces/row spaces/column spaces, exact affine parametrizations of rectangular linear systems, derivatives/integrals, multivariate partials/gradients/Hessians, polynomial-map composition/Jacobians/divergence/curl, polynomial-form wedge/exterior derivatives/pullbacks/insertion/Lie derivatives, finite-set operations, moments/covariances and conditioning.
...
| 6 Calculus | Native polynomial Euclidean arithmetic and calculus, rational-function field/differentiation/composition, exact multivariate partials/gradient/Hessian/Laplacian, polynomial-map composition/Jacobian/divergence/curl; polynomial-form exterior derivative and pullback; primitive-family utility | Certified general function spaces, multivariate rational functions and manifold calculus |
...
| 9 Geometry/topology | Native finite simplicial algebra, skeletons, Euler characteristic and F2 Betti numbers; polynomial coordinate differential forms, exterior differentiation, pullbacks and Cartan Lie derivatives | Manifold charts, general smooth forms/metrics, general form integration and integral/persistent homology |
...
The initial registry has 1023 scoped records, covering all 322 distinct requested bullet concepts in specification sections 10-30, 48 domain descriptors and nine extension proposals. These are survey counts, not a percentage of all mathematics. All records remain human-unreviewed and formally unverified.
