...
The default initializer has 28 algebra builders and 417 native registrations. [Concrete examples](CONCRETE_ALGEBRAS.md) cover arithmetic, finite sets/functions/categories, statistics, probability, vectors, rectangular matrices, affine solution sets, exact least-squares and polynomials. Vec(Q) and Mat(Q) are families with partial dimension-sensitive operations; they are not one vector space or ring across all dimensions. Pseudoinverses and projections use standard Euclidean inner products over rational coordinates. Q(i) is a proper subfield of C. Finite symbolic expressions are not all R. Polynomial differentiation does not decide differentiability of arbitrary callbacks.
