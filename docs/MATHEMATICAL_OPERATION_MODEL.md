...
RelativeComplex retains a labelled pair (X,A) with A a subcomplex of X. Its quotient chains use the ordered simplices outside A, and degreewise homology returns the existing IntegralHomology carrier. Inclusion and quotient are chain maps; the zero-on-A lift is only a chain-group section. The connecting map takes the lifted boundary class in H_(k-1)(A), with the unreduced convention H_-1(A)=0. Scalar maps and the flat three-map long-exact-segment return AbelianGroupHomomorphism wrappers with compatible retained presentations. Pair construction uses ICustomResultOperation, counts use second N wrappers, and foreign-result scalar/flat operations reuse the existing mixed and unary-transfer implementations. All compound computations, including complete degree lists and three-map segments, share one integer work budget.

RelativeMap retains a simplicial ambient map and both labelled RelativeComplex boundaries, and validates that its restriction carries A into B simplex by simplex. From-map consumes the existing SimplicialMap and a validated source/target pair carrier through a native mixed operation. Composition requires equality of both middle components; inversion requires an isomorphism of pairs. Relative chain matrices discard collapsed simplices and images in B using the existing orientation implementation. Scalar and flat relative homology maps return AbelianGroupHomomorphism wrappers; long-exact-maps emits the four vertical maps compatible with the existing three-map segments. Pair contiguity checks the subcomplex condition as well as ambient contiguity. A single integer work budget covers each induced map, whole degree list or four-map naturality list.

SimplicialCover retains two ordered finite subcomplexes and takes their simplex-wise union as the ambient complex. Its block-diagonal chain complex uses left then right coordinates and returns the existing IntegralHomology carrier, with explicit component inclusions/projections identifying its homology as a direct sum. The signed intersection map (i,-j), addition into the union and the boundary of the left part give the three Mayer-Vietoris homomorphisms in existing AbelianGroupHomomorphism wrappers. The deterministic chain splitting is not generally a homology splitting. Excision returns the existing RelativeMap inclusion (A,A intersection B)->(A union B,B), whose quotient chain bases agree. Combined chain ranks obey the matrix bound, and whole degree lists and exact-sequence segments share one integer work budget.

CoverMap retains a union SimplicialMap and both ordered SimplicialCover boundaries, validating simplex preservation in each piece. Its block-diagonal sum maps use the existing chain bases and return Mat(Z) and AbelianGroupHomomorphism wrappers. The flat four-map naturality operation commutes with the existing Mayer-Vietoris homomorphisms; chain splittings need not commute. Unary scalar/flat transfers return the two RelativeMap values commuting with excision. Construction uses a validated source/target cover-pair carrier and IUnsafeOperation; restriction returns the first CoverMap wrapper through ICustomMemberOperation. All operations reuse operations/simple and operations/flat and compose in the original AlgebraFlow. Combined sum ranks obey the matrix bound, and each whole degree list or four-map naturality list shares one integer work budget.

The same SimplicialCover and CoverMap algebras now expose cohomological Mayer-Vietoris. Transposed boundaries define sum cohomology in the existing IntegralHomology carrier; restriction, left-minus-right difference and degree-raising connecting maps yield an exact integral sequence with torsion. Cover morphisms induce contravariant maps and four-map naturality lists, reversing composition. Full presentations are retained, and component cohomology inclusions/projections identify the sum. A deterministic connecting cochain matrix need not be natural before passage to cohomology. All 30 added scalar/flat operations reuse the existing operations/simple and operations/flat wrappers and original MathTool/AlgebraFlow. Whole degree lists and exact/naturality segments share the integer work budget, including every quotient model they construct.

SimplicialCochain retains a labelled complex, a nonnegative degree and integral coordinates dual to its oriented simplex basis. It is a family of homogeneous groups: addition checks both context and degree, while cup multiplication adds degrees. Coboundary raises the degree and squares to zero. The existing IntegralHomology carrier computes its kernel/quotient from transposed boundaries in reversed order, so AbelianGroupElement and AbelianGroupHomomorphism continue to carry actual presentations, torsion and induced maps. Unary flat cocycle-generators returns wrapped cochains; scalar cup-class returns the quotient element after checking both inputs are cocycles. Pullback, coefficient replacement, scaling and representative selection return the first cochain carrier through ICustomMemberOperation. Simplicial-map cohomology transfers reverse source and target. No new execution engine is introduced; all scalar and flat operations use the original operations interfaces and AlgebraFlow.

RelativeCochain retains a full labelled pair and coordinates dual to its quotient-chain basis, with the subcomplex filtered out before matrix-size checks. Its cup product accepts (X,A) and (X,B) and returns a cochain on (X,A union B); addition requires the same full pair and degree. Absolute conversion and extension by zero connect the original SimplicialCochain algebra. Existing IntegralHomology, AbelianGroupElement and AbelianGroupHomomorphism carriers supply relative cohomology, classes and maps. Scalar/flat operations expose the three maps H^k(X,A)->H^k(X)->H^k(A)->H^(k+1)(X,A) and the four contravariant maps induced by a RelativeMap. The connecting cochain formula is natural only after taking classes. Complete degree lists, exact segments and naturality lists share one integer work budget, and all execution uses the existing operations interfaces and AlgebraFlow.


SimplicialChain is a native family of homogeneous integral chains retaining a full labelled complex and integer degree. Negative-degree groups are zero under the unreduced convention; boundary lowers degree, while augmentation is a separate zero-chain operation. Cap with SimplicialCochain evaluates front faces and retains back faces, returning the first chain wrapper through ICustomMemberOperation. Pairing checks both contexts and degrees. Fixed cocycles induce H_n->H_(n-p), and fixed cycles induce H^p->H_(n-p), using the existing IntegralHomology and AbelianGroupHomomorphism algebras and preserving torsion. Both models and induction share a bounded integer computation. Sorted chain representatives need not satisfy cap naturality for arbitrary vertex maps; their homology classes do. These operations consume explicit cycles and do not infer manifold status or certify a fundamental class. Unary boundary, flat cycle generators and serialized cap flows execute through the original operation interfaces.


RelativeChain retains a full simplicial pair, integer degree and quotient-chain coordinates outside its subcomplex. Projection from SimplicialChain is a chain map; zero-on-A lifting is only a chain-group section. Typed connecting cycles land on the full subcomplex. Absolute cochains act by cap within RelativeChain through ICustomMemberOperation; relative cochains on the same pair cap into SimplicialChain through the existing mixed-result interface. Fixed cocycles induce maps from relative homology, while fixed relative cycles induce maps from absolute or relative cohomology with the appropriate relative or absolute target. Matrices with a fixed cochain use the context chain's pair and degree and ignore its coefficients. Full presentations and shared computation budgets are retained. The shared front/back matrix implementation filters omitted front/back faces according to their different mathematical roles. Supplied cycles do not certify fundamental classes or manifold duality.

RelativeCap binds a RelativeChain on (X,D) to an explicit target pair (X,B), with B contained in D. A RelativeCochain on (X,A) is accepted only when all full labelled ambient complexes match and D=A union B. This context expresses the general relative cap product through the existing binary mixed-result operation, returning a RelativeChain on (X,B). Both matrices and induced maps support fixing either argument; cohomology operations take a cochain as a pair-and-degree template whose coordinates are ignored. The class and map calculations retain integer quotient presentations and use a shared work budget. The original unary boundary and custom-member chain replacement operate on the context, and serialized flows retain the explicit target.

RelativeTriple retains B subset A subset X and its three quotient pairs. Native selector matrices form the short exact chain sequence C(A,B)->C(X,B)->C(X,A); transposes give the dual cochain sequence. The existing IntegralHomology and AbelianGroupHomomorphism carriers construct the integral long exact sequences, preserving presentations and torsion. Typed connecting cycles and cocycles return the actual second operand wrappers through ILeftProjectionOperation, with checked full source pairs and changed result pairs/degrees. Scalar maps share one integer work budget across both models and induction; each flat three-map segment shares one budget across all four models and all maps. Matrix and sequence degrees are nonnegative, while typed chains retain negative zero groups.

TripleMap retains a full source and target RelativeTriple and a simplicial map preserving both nested components. It exposes three actual RelativeMaps, so chain matrices, pushforwards and pullbacks continue through existing carriers. Homology maps are covariant; cohomology maps are contravariant and retain full quotient presentations, including killed Smith coordinates. The two flat four-map lists implement the naturality diagrams in their documented orders, sharing one work budget across all models and maps. At degree zero, homology uses the typed degree-minus-one presentations rather than replacing them by a canonical zero group. Composition, restriction and inversion check all three components; contiguity checks image unions separately in the ambient, middle and base complexes. Execution remains on the original scalar, unary and flat operation interfaces.

SimplicialHomotopy retains two ordered contiguous RelativeMaps on the same full source and target pairs. Its oriented prism raises chain degree and its transpose lowers cochain degree, witnessing the difference of the endpoint chain maps and pullbacks. Absolute construction embeds simplicial maps as pairs with empty subcomplexes. Chain and cochain actions return the actual second operand wrappers through ILeftProjectionOperation, retaining changed geometric contexts and degrees. Matrix lists use the original unary flat transfers with a shared computation budget. Degree-zero cochain matrices retain their zero-row shape; typed cochain actions require positive degree because the existing cochain carrier excludes negative degrees. This carrier represents one contiguous pair and does not search for general homotopies.

HomotopyPath retains a finite supplied sequence of RelativeMaps with identical full boundaries and consecutive contiguity in both target components. It accumulates the existing prism witnesses under one work budget, giving explicit chain/cochain homotopies between possibly noncontiguous endpoints. Chronological concatenation checks full joining-map equality; stationary paths are units, and stage equality retains repetitions and nontrivial loops. Appending and pre/postcomposition use ICustomMemberOperation; typed actions keep ILeftProjectionOperation and actual second operand wrappers. Pre/postcomposition revalidates all new stages with the same budget used for composition. The resulting sorted prisms are recomputed, so precomposition and reversal need not multiply or negate the earlier matrices. Whole flat matrix lists share one budget across every stage and degree.
