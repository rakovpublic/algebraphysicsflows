...
class CoverageTest(unittest.TestCase):
    ...
    def test_operation_conditions_are_scoped_to_the_algebra(self):
        ...
        self.assertIn("input need not vanish on A", " ".join(records["RelativeChain.from-absolute"]["required_invariants"]))
        self.assertIn("generally not a chain map", " ".join(records["RelativeChain.lift-absolute"]["required_invariants"]))
        self.assertIn("full subcomplex A", " ".join(records["RelativeChain.connect-cycle"]["required_invariants"]))
        self.assertIn("same pair", " ".join(records["RelativeChain.cap"]["required_invariants"]))
        self.assertIn("back faces in A must be retained", " ".join(records["RelativeChain.relative-cap"]["required_invariants"]))
        self.assertIn("H^p(X,A)->H_(n-p)(X)", " ".join(records["RelativeChain.relative-cap-cohomology-map"]["required_invariants"]))
        self.assertIn("ignore its coordinates", " ".join(records["RelativeChain.relative-cap-matrix"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["RelativeChain.relative-cap-homology-map"]["known_limitations"]))
        self.assertIn("B contained in D", " ".join(records["RelativeCap.on"]["required_invariants"]))
        self.assertIn("D exactly equal to A union B", " ".join(records["RelativeCap.cap"]["required_invariants"]))
        self.assertIn("outside B", " ".join(records["RelativeCap.cap"]["required_invariants"]))
        self.assertIn("ignore the stored chain coordinates", " ".join(records["RelativeCap.cap-matrix"]["required_invariants"]))
        self.assertIn("need not be a cocycle", " ".join(records["RelativeCap.cap-cohomology-map"]["required_invariants"]))
        self.assertIn("H^p(X,A)->H_(n-p)(X,B)", " ".join(records["RelativeCap.cap-cohomology-map"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["RelativeCap.cap-homology-map"]["known_limitations"]))
        self.assertIn("B contained in A", " ".join(records["RelativeTriple.from-pair"]["required_invariants"]))
        self.assertIn("generally not a chain map", " ".join(records["RelativeTriple.lift-matrix"]["required_invariants"]))
        self.assertIn("H_k(X,A)->H_(k-1)(A,B)", " ".join(records["RelativeTriple.connecting-homology"]["required_invariants"]))
        self.assertIn("H^k(A,B)->H^(k+1)(X,A)", " ".join(records["RelativeTriple.connecting-cohomology"]["required_invariants"]))
        self.assertIn("ILeftProjectionOperation", " ".join(records["RelativeTriple.connect-cycle"]["required_invariants"]))
        self.assertIn("full inner pair", " ".join(records["RelativeTriple.connect-cocycle"]["required_invariants"]))
        self.assertIn("entire list", " ".join(records["RelativeTriple.long-exact-segment"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["RelativeTriple.long-exact-cohomology-segment"]["known_limitations"]))
        self.assertIn("B maps into D", " ".join(records["TripleMap.from-map"]["required_invariants"]))
        self.assertIn("all three", " ".join(records["TripleMap.inclusion"]["required_invariants"]))
        self.assertIn("middle and base", " ".join(records["TripleMap.contiguous"]["required_invariants"]))
        self.assertIn("four source-to-target", " ".join(records["TripleMap.long-exact-maps"]["required_invariants"]))
        self.assertIn("degree-minus-one presentations", " ".join(records["TripleMap.long-exact-maps"]["required_invariants"]))
        self.assertIn("four target-to-source", " ".join(records["TripleMap.long-exact-cohomology-maps"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["TripleMap.long-exact-cohomology-maps"]["known_limitations"]))
        self.assertIn("both the ambient target and its subcomplex", " ".join(records["SimplicialHomotopy.between"]["required_invariants"]))
        self.assertIn("to# - from#", " ".join(records["SimplicialHomotopy.chain-matrix"]["required_invariants"]))
        self.assertIn("zero rows", " ".join(records["SimplicialHomotopy.cochain-matrix"]["required_invariants"]))
        self.assertIn("ILeftProjectionOperation", " ".join(records["SimplicialHomotopy.on-chain"]["required_invariants"]))
        self.assertIn("positive degree", " ".join(records["SimplicialHomotopy.on-cochain"]["required_invariants"]))
        self.assertIn("empty subcomplexes", " ".join(records["SimplicialHomotopy.on-absolute-chain"]["required_invariants"]))
        self.assertIn("entire list", " ".join(records["SimplicialHomotopy.chain-matrices"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["SimplicialHomotopy.cochain-matrices"]["known_limitations"]))
        self.assertIn("zero-step path", " ".join(records["HomotopyPath.stationary-on"]["required_invariants"]))
        self.assertIn("equal full joining maps", " ".join(records["HomotopyPath.then"]["required_invariants"]))
        self.assertIn("including repeated stages", " ".join(records["HomotopyPath.stages"]["required_invariants"]))
        self.assertIn("to# - from#", " ".join(records["HomotopyPath.chain-matrix"]["required_invariants"]))
        self.assertIn("every degree and every step", " ".join(records["HomotopyPath.chain-matrices"]["required_invariants"]))
        self.assertIn("ILeftProjectionOperation", " ".join(records["HomotopyPath.on-chain"]["required_invariants"]))
        self.assertIn("need not equal", " ".join(records["HomotopyPath.precompose"]["required_invariants"]))
        self.assertIn("composition/validation budget", " ".join(records["HomotopyPath.postcompose"]["required_invariants"]))
        self.assertIn("256 stages", " ".join(records["HomotopyPath.append"]["known_limitations"]))
        self.assertIn("Id_X -> g after f and Id_Y -> f after g", " ".join(records["HomotopyEquivalence.from-maps"]["required_invariants"]))
        self.assertIn("does not yet require opposite boundaries", " ".join(records["HomotopyEquivalence.maps"]["required_invariants"]))
        self.assertIn("both transported/concatenated paths", " ".join(records["HomotopyEquivalence.compose"]["required_invariants"]))
        self.assertIn("Do not reverse either path", " ".join(records["HomotopyEquivalence.inverse"]["required_invariants"]))
        self.assertIn("both complete witness stage lists", " ".join(records["HomotopyEquivalence.equal"]["required_invariants"]))
        self.assertIn("exactly [f_*,g_*]", " ".join(records["HomotopyEquivalence.homology-maps"]["required_invariants"]))
        self.assertIn("exactly [f^*,g^*]", " ".join(records["HomotopyEquivalence.cohomology-maps"]["required_invariants"]))
        self.assertIn("H^n(Y,B)->H^n(X,A)", " ".join(records["HomotopyEquivalence.forward-cohomology-map"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["HomotopyEquivalence.compose"]["known_limitations"]))
        self.assertIn("ILeftProjectionFlatOperation", " ".join(records["HomotopyEquivalence.dominators"]["required_invariants"]))
        self.assertIn("subcomplex", " ".join(records["HomotopyEquivalence.dominators"]["required_invariants"]))
        self.assertIn("pair-relative minimality", " ".join(records["HomotopyEquivalence.is-strong-core"]["required_invariants"]))
        self.assertIn("one-step source path", " ".join(records["HomotopyEquivalence.collapse-vertex"]["required_invariants"]))
        self.assertIn("least compatible dominator", " ".join(records["HomotopyEquivalence.strong-core"]["required_invariants"]))
        self.assertIn("no partial core", " ".join(records["HomotopyEquivalence.strong-core"]["required_invariants"]))
        self.assertIn("255 deletions", " ".join(records["HomotopyEquivalence.strong-core-absolute"]["known_limitations"]))
