...
class CoverageTest(unittest.TestCase):
    ...
    def test_operation_conditions_are_scoped_to_the_algebra(self):
        ...
        self.assertIn("(i,-j)", " ".join(records["SimplicialCover.intersection-matrix"]["required_invariants"]))
        self.assertIn("generally not a chain map", " ".join(records["SimplicialCover.split-matrix"]["required_invariants"]))
        self.assertIn("H_-1(I)=0", " ".join(records["SimplicialCover.connecting-homology-map"]["required_invariants"]))
        ...
        self.assertIn("left minus right", " ".join(records["SimplicialCover.difference-matrix"]["required_invariants"]))
        self.assertIn("H^(k+1)(U)", " ".join(records["SimplicialCover.connecting-cohomology-map"]["required_invariants"]))
        self.assertIn("entire three-map segment", " ".join(records["SimplicialCover.long-exact-cohomology-segment"]["required_invariants"]))
        self.assertIn("including torsion", " ".join(records["SimplicialCover.sum-cohomology"]["required_invariants"]))
        self.assertIn("composition reverses order", " ".join(records["CoverMap.sum-cohomology-map"]["required_invariants"]))
        self.assertIn("four target-to-source", " ".join(records["CoverMap.long-exact-cohomology-maps"]["required_invariants"]))
        self.assertIn("entire list", " ".join(records["CoverMap.sum-cohomology-maps"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["CoverMap.long-exact-cohomology-maps"]["known_limitations"]))
        self.assertIn("zero group in degree -1", " ".join(records["SimplicialChain.boundary"]["required_invariants"]))
        self.assertIn("first carrier", " ".join(records["SimplicialChain.cap"]["required_invariants"]))
        self.assertIn("equal degrees", " ".join(records["SimplicialChain.evaluate"]["required_invariants"]))
        self.assertIn("cycle and a cocycle", " ".join(records["SimplicialChain.cap-class"]["required_invariants"]))
        self.assertIn("H_n->H_(n-p)", " ".join(records["SimplicialChain.cap-homology-map"]["required_invariants"]))
        self.assertIn("map need not be an isomorphism", " ".join(records["SimplicialChain.cap-cohomology-map"]["required_invariants"]))
        self.assertIn("5000000", " ".join(records["SimplicialChain.cap-cohomology-map"]["known_limitations"]))
        self.assertIn("not generally natural on chains", " ".join(records["SimplicialChain.cap"]["known_limitations"]))
